...
// Google Fonts Developer API key, restricted (by the project owner, in the Google Cloud
...
function fetchGoogleFontsList(){
  ...
  __googleFontsListPromise = fetch(url)
...
function getOpentypeFont(family, weight, italic){
  ...
  const promise = fetchGoogleFontsList().then(list => {
    ...
    const fileUrl = findFontVariantFileUrl(meta, weight, italic);
    if (!fileUrl) throw new Error(`No usable font file for "${family}"`);
    // Google's webfonts API returns plain http:// file URLs; force https so the fetch
    // isn't blocked as mixed content when Droidwright itself is served over https.
    return fetch(fileUrl.replace(/^http:\/\//, 'https://'));
  }).then(r => {
...
function regenerateTextPath(shape){
  ...
  return getOpentypeFont(shape.fontFamily, weight, italic).then(font => {
...
function renderSelectionOverlay(){
  ...
  if (state.activeEndpointSnap){
    ...
    badge.textContent = 'Snap & Connect';
...
function onKeyDown(e){
  ...
  const toolKeys = { v:'select', n:'node', r:'rect', o:'ellipse', a:'arc', p:'polygon', l:'line', c:'curve', f:'pen', h:'pan', k:'cut', t:'text' };
